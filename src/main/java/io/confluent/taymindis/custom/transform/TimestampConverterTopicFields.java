...
public abstract class TimestampConverterTopicFields<R extends ConnectRecord<R>>
  ...
  public static final String OVERVIEW_DOC =
      "Convert timestamps between different formats such as Unix epoch, strings, and Connect Date/Timestamp types."
          ...
          + TimestampConverterTopicFields.Key.class.getName() + "</code>) "
          + "or value (<code>" + TimestampConverterTopicFields.Value.class.getName() + "</code>).";

  public static final String FIELD_CONFIG = "field";
  public static final String FIELDS_BY_TOPIC = "fieldsByTopic";
  public static final String FIELDS_BY_TOPIC_PREFIX = "fieldsByTopicPrefix";
  ...
  public static final ConfigDef CONFIG_DEF = new ConfigDef()
      ...
      .define(FIELDS_BY_TOPIC_PREFIX, ConfigDef.Type.STRING, FIELD_DEFAULT, ConfigDef.Importance.LOW,
          "The fields by topic prefix containing the timestamp")
      .define(FIELDS_BY_TOPIC, ConfigDef.Type.LIST, Collections.emptyList(), ConfigDef.Importance.LOW,
          "The fields by topic containing the timestamp, or empty if the entire value is a timestamp")
  ...
  private static class Config {
    Config(String field, String fieldByTopicPrefix, Map<String, List<String>> fieldsByTopic, String type, SimpleDateFormat format) {
      this.field = field;
      this.fieldByTopicPrefix = fieldByTopicPrefix;
      this.fieldsByTopic = fieldsByTopic;
      this.type = type;
      this.format = format;
    }

    String field;
    String fieldByTopicPrefix;
    Map<String, List<String>> fieldsByTopic;
    String type;
  ...
  public void configure(Map<String, ?> configs) {
    ...
    final String fieldsByTopicPrefix = simpleConfig.getString(FIELDS_BY_TOPIC_PREFIX);
    final List<String> fieldsMappedTopic = simpleConfig.getList(FIELDS_BY_TOPIC);
    ...
    Map<String, List<String>> fieldsByTopic = new HashMap<>();

    for (String fields : fieldsMappedTopic) {
      String[] topicField = fields.split(":");
      ...
      fieldsByTopic.put(fieldsByTopicPrefix.concat(topicField[0]), Arrays.asList(topicField[1].split("\\|")));
    }


    config = new Config(field, fieldsByTopicPrefix, fieldsByTopic, type, format);
  }
  ...
  public static class Key<R extends ConnectRecord<R>> extends TimestampConverterTopicFields<R> {
    @Override
  ...
  public static class Value<R extends ConnectRecord<R>> extends TimestampConverterTopicFields<R> {
    @Override
    protected Schema operatingSchema(R record) {
  ...
  private R applyWithSchema(R record) {
    ...
    if (config.field.isEmpty() && config.fieldsByTopic.isEmpty()) {
      ...
      if (updatedSchema == null) {
        ...
        if (!config.fieldsByTopic.isEmpty()) {
          List<String> targetFields = config.fieldsByTopic.getOrDefault(record.topic(), emptyList);
        ...
        if (schema.defaultValue() != null) {
          Struct updatedDefaultValue = applyValueWithSchema((Struct) schema.defaultValue(), builder, record);
      ...
      Struct updatedValue = applyValueWithSchema(value, updatedSchema, record);
      return newRecord(record, updatedSchema, updatedValue);
    }
  }

  private Struct applyValueWithSchema(Struct value, Schema updatedSchema, R record) {
    if (value == null) {
      return null;
    }
    ...
    if (!config.fieldsByTopic.isEmpty()) {
      List<String> fields = config.fieldsByTopic.getOrDefault(record.topic(), emptyList);
  ...
  private R applySchemaless(R record) {
    ...
    if (rawValue == null || (config.field.isEmpty() && config.fieldsByTopic.isEmpty())) {
